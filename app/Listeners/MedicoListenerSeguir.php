...
class MedicoListenerSeguir
{
     ...
     * Handle the event.
     *
     * @param  MedicoEventSeguir  $event
     * @return void
     */
    public function handle(MedicoEventSeguir $event)
    {
         //guardamos actividad
        $user=$event->data;
        ...
        $dataActividad=[
                        'descripcion'=>auth()->user()->name.' '.$user['descripcion'].' "'. $medicoName.'"  '.$mytime->toTimeString(),
                        'iduser'=>$user['iduser'],
                        'last_logged_at'=> null,
                        'last_login'=> null,
                        'idarticulo'=>null,
